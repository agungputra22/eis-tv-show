window.HISTORICAL_MUTASI = (function($) {

    return {

        elTable : '#datatables',
        elTableServer : '#datatable_ajax',
        urlGetData : window.APP.siteUrl + 'historical_mutasi/datalist',

        elPortlet : '.portlet',
        elForm : '.form-historical_mutasi',
        elBtnList : '.btnList',
        ...
        handleDeleteData : function() {
            ...
            $(parentThis.elBtnDelete).click(function() {
                if(confirm('Apakah anda akan set tanggal efektif ?')) {
                    $.ajax({
                        url : $(this).data('url'),
                        type : 'post',
                        dataType : 'json',
        ...
        handleCloseData : function() {
            var parentThis = this;
            $(parentThis.elBtnClose).click(function() {
                if(confirm('Apakah anda ingin close data ?')) {
                    $.ajax({
                        url : $(this).data('url'),
                        type : 'post',
                        dataType : 'json',
        ...
        initLoad : function(btn) {
            var url = window.APP.siteUrl + 'historical_mutasi/index';
            Layout.loadAjaxContent(url, btn);
        },

        initLoad_approve : function(btn) {
            var url = window.APP.siteUrl + 'historical_mutasi/index_approve';
            Layout.loadAjaxContent(url, btn);
        },
