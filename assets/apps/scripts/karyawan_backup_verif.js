window.KARYAWAN_BACKUP_VERIF = (function($) {

    return {

        elTable : '#datatables',
        elTableServer : '#datatable_ajax',
        urlGetData : window.APP.siteUrl + 'karyawan_backup_verif/datalist',

        elPortlet : '.portlet',
        elForm : '.form-karyawan_backup_verif',
        elBtnList : '.btnList',
        ...
        handleVerif : function() {
            var parentThis = this;
            $(parentThis.elBtnVerif).click(function() {
                if(confirm('Apakah anda ingin Verifikasi ?')) {
                    $.ajax({
                        url : $(this).data('url'),
                        type : 'post',
                        dataType : 'json',
