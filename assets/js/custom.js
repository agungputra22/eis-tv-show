...
function jadiSimpan(obj){
    if($('#saving-loader').is(':visible')==false){
        var form = $(obj).parent().parent().parent().find('form');
        var nik = $('#nik_baru').val().trim();
        var nama = $('#nama').val().trim();
        var thn = $('#tahun-lahir').val();
        var jabatan = $('#jabatan_karyawan_shift').val();
        var dept = $('#dept_karyawan_shift').val();
        var jam_shift = $('#jam_shift').val();
        var bln_tgl = $('#bulan-dan-tanggal').val();
        var keterangan_shift = $('#keterangan_shift').val();
        var no_co_shift = $('#no_co_shift').val();
        if(nik == "" || nama=="" || thn=="" || jabatan=="" || dept=="" || jam_shift=="" || keterangan_shift==""){
            alert('Semua input harus diisi!');
        }
        else{
            var fulldate = thn+'-'+bln_tgl;
            simpanDataBaru(form, nik, nama, fulldate, jabatan, dept, jam_shift, keterangan_shift, no_co_shift);           
        }
    }
    else{
...
function simpanDataBaru(form, nik, nama, fulldate, jabatan, dept, jam_shift, keterangan_shift, no_co_shift){
    $('#saving-loader').show();
    $.ajax({
        url: 'kalender/create_event/',
        type: "POST",
        data: {nik_shift: nik, name: nama, birth_date: fulldate, jabatan_karyawan_shift: jabatan, dept_karyawan_shift: dept, jam_shift: jam_shift, keterangan_shift: keterangan_shift, no_co_shift: no_co_shift},
